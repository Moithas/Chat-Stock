...
const BUTTON_IDS = [
  ...
  'admin_pets_economy', 'admin_pets_kennel',
  'back_admin_pets',
];
...
async function showPetsAdminPanel(interaction, guildId) {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: '🥚 Egg Prices', value: `Mystery: ${settings.eggMysteryPrice.toLocaleString()}\nGolden: ${settings.eggGoldenPrice.toLocaleString()}\nPrismatic: ${settings.eggPrismaticPrice.toLocaleString()}`, inline: false },
      { name: '🏠 Kennel Prices', value: `L1: ${settings.kennelL1Price.toLocaleString()}\nL2: ${settings.kennelL2Price.toLocaleString()}\nL3: ${settings.kennelL3Price.toLocaleString()}`, inline: false },
    )
    .setTimestamp();
  ...
  const row2 = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('admin_pets_kennel').setLabel('Kennel/Egg Prices').setEmoji('🏠').setStyle(ButtonStyle.Primary),
